    ...
    public ByteBuffer allocate(int size, long maxTimeToBlockMs) throws InterruptedException {
        ...
        try {
            ...
            if (this.availableMemory + freeListSize >= size) {
                ...
                lock.unlock();//解锁
                return ByteBuffer.allocate(size);//这里并没有使用free队列中的buffer,而是直接分配size大小的HeapByteBuffer
            } else {
                ...
                while (accumulated < size) {//循环等待
                    ...
                    try {
                        ...
                        this.waiters.remove(moreMemory);//出现异常,移除此线程对应的Condition
                        throw e;
                    } finally {
                        long endWaitNs = time.nanoseconds();//统计阻塞时间
